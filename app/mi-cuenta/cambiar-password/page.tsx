...
export default function CambiarPasswordPage() {
  ...
  return (
    <div>
      <h2 className="text-xl font-bold text-stone-900 mb-6">Cambiar Contraseña</h2>
      ...
      <form onSubmit={handleSubmit} className="space-y-4 max-w-md">
        <div>
          ...
          <div className="relative">
            <input
              ...
              className="w-full px-3 py-2 pr-10 border border-stone-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-emerald-500"
              required
            />
            <button
              type="button"
        ...
        <div className="bg-stone-50 rounded-lg p-4 space-y-2">
          <p className="text-sm font-medium text-stone-700 mb-2">La nueva contraseña debe tener:</p>
          {passwordRules.map((rule) => {
            const ok = rule.test(formData.newPassword)
            return (
              <div key={rule.label} className="flex items-center gap-2 text-sm">
                <span className={`w-2 h-2 rounded-full ${ok ? 'bg-emerald-500' : 'bg-stone-300'}`} />
                <span className={ok ? 'text-stone-900' : 'text-stone-500'}>{rule.label}</span>
              </div>
            )
          })}
        </div>
