...
export default function AdminEmpleados() {
  ...
  if (!isAdmin) {
    return (
      <div className="p-8 flex flex-col items-center justify-center min-h-[400px] text-center">
        ...
        <p className="text-stone-400">Solo los administradores pueden ver esta sección</p>
      </div>
    )
  }
  ...
  return (
    <div className="p-6 lg:p-8 max-w-7xl mx-auto">
      <div className="mb-6">
        <h1 className="text-xl font-bold text-stone-900 dark:text-white">Empleados</h1>
        <p className="text-stone-500 dark:text-stone-400 text-sm">{empleados.length} empleados registrados</p>
      </div>
                ...
                return (
                  <tr key={e.id_empleado} className="hover:bg-stone-50 dark:hover:bg-stone-800/50 transition-colors">
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-3">
                        <div className="w-8 h-8 bg-stone-100 dark:bg-stone-800 rounded-full flex items-center justify-center flex-shrink-0">
                          <span className="text-stone-600 dark:text-stone-300 text-xs font-bold uppercase">{e.nombre?.[0]}</span>
                        </div>
                    ...
                    <td className="px-4 py-3 text-center hidden sm:table-cell">
                      <span className={`text-[11px] font-medium px-2 py-1 rounded-full ${e.estado === 'activo' ? 'text-emerald-400 bg-emerald-900/30' : 'text-stone-500 bg-stone-800'}`}>
                        {e.estado || '—'}
                      </span>
                    </td>
                    <td className="px-4 py-3 text-center hidden md:table-cell">
                      <select
                        value={e.cargo || ''}
                        disabled={cambiando === e.id_empleado}
                        onChange={ev => cambiarCargo(e.id_empleado, ev.target.value)}
                        className="bg-stone-50 dark:bg-stone-800 border border-stone-300 dark:border-stone-700 text-stone-700 dark:text-stone-300 text-xs rounded-lg px-2 py-1.5 focus:outline-none focus:ring-1 focus:ring-emerald-500 disabled:opacity-50"
                      >
